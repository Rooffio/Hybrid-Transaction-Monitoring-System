...
NUM_CUSTOMERS = 23_000
OUTPUT_FILE = "data/raw/normalized_tms_extract.csv"
...
# Set a seed for predictable randomness in profile generation if desired
# random.seed(42)
# np.random.seed(42)
...
def generate_tms_data():
    ...
    # --- 1. PRE-GENERATE ENHANCED CUSTOMER PROFILES (Recalibrated for USD) ---
    ...
    for _ in tqdm(range(NUM_CUSTOMERS), desc="Creating KYC/STR Master List"):
        ...
        # Recalibrated Statistical Baselines for USD
        # Format: (Mean Transaction Value, Std Deviation)
        stats = {
            "STUDENT": (40, 25),  # Small P2P, coffee, books
            "RETAIL": (150, 90),  # Groceries, utilities, shopping
            "SME": (2500, 1200),  # B2B payments, inventory
            "CORPORATE": (45000, 20000),  # Large transfers, payroll
            "HNI": (8000, 3500)  # High Net Worth individuals
        }
        avg, std = stats[group]
        ...
        customer_db[u_id] = {
            ...
            "peer_group": group,
            "expected_monthly_vol": avg * 8,  # Estimate 8 txns/month
    ...
    # --- 2. GENERATE TRANSACTION LOGS WITH USD OUTLIERS ---
    ...
    with open(OUTPUT_FILE, mode='w', newline='') as f:
        ...
        for i in tqdm(range(NUM_TRANSACTIONS), desc="Synthesizing Transactions"):
            ...
            # Network Graph: Artificial Hubs for Centrality
            if i % 10 == 0:
                r_id = customer_ids[0]  # The "Super-Mule" hub
            else:
            ...
            # Base Amount Logic (Normal Distribution)
            ...
            # Integration logic for Asset Purchases
